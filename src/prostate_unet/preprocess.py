...
def resample_to_spacing(
    ...
    """
    Reample a 3D volume to the target voxel spacing using scipy.ndimage.zoom

    :param vol: (Z, Y, X) ndarray
    :param current_spacing: (sz, sy, sx) in mm
    :param target_spacing: (tz, ty, tx) in mm
    :param order: 1 for images, 0 for labels (nearest neighbor)
    :return: Resampled volume as ndarray
    """

    cs = np.array(current_spacing, dtype=np.float64)
    ts = np.array(target_spacing, dtype=np.float64)

    # zoom factor = old_spacing / new_spacing (e.g. 2.0mm -> 1.0 mm => 2.0x)
    factors = cs / ts
...
def bbox_from_mask(mask: np.ndarray, margin_vox: Sequence[int]) -> Tuple[slice, slice, slice]:
    ...
    Returns a tuple of slices usable for vol[z, x , y] indexing
    :param mask: ndarray  of mask
    :param margin_vox: margin in voxels
    :return: tuple of slices for indexing
    """
    assert mask.ndim == 3
    # Create array of coordinates where there is a mask
    coords = np.argwhere(mask > 0)
    if coords.size == 0:
        # no foreground: return full volume
        return slice(0, mask.shape[0]), slice(0, mask.shape[1]), slice(0, mask.shape[2])

    # Compute dimensions for smallest box fitting the mask plus margin
    zmin, ymin, xmin = coords.min(axis=0)
    zmax, ymax, xmax = coords.max(axis=0)
    mz, my, mx = margin_vox
    z0 = max(0, zmin - mz);
    z1 = min(mask.shape[0], zmax + mz + 1)
    y0 = max(0, ymin - my);
    y1 = min(mask.shape[1], ymax + my + 1)
    x0 = max(0, xmin - mx);
    x1 = min(mask.shape[2], xmax + mx + 1)
...
def preprocess_pair(
    ...
    Full pipeline for one (image,label) pair:
      ...
      4) return arrays + metadata ready to cache
    """
    # 1) orient to RAS
    ...
    # 2) resample
    cur_sz = voxel_sizes(img_ras)
    # Build label mask (>0 or specific classes)
    ...
    # Intensity normalization (simple z-score inside the crop; optional)
